  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-xl font-semibold text-[var(--text)] mb-2">Document verification</h2>
        <p className="text-sm text-[var(--text-muted)]">Upload required ID documents</p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">Primary ID (Aadhaar or Voter)</label>
          <select
        ...
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">Primary ID Number</label>
          <input
        ...
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">Upload Primary ID document</label>
          <input
            type="file"
            accept="image/*,.pdf"
            onChange={(e) => handleFileChange(e, setPrimaryFile)}
            className="w-full px-3 py-2 bg-[var(--bg)] border border-[var(--border)] rounded-lg text-[var(--text)] file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:bg-[var(--accent)] file:text-white"
          />
          {!primaryFile && fileErrors.primaryFile && <p className="text-red-400 text-xs mt-1">{fileErrors.primaryFile}</p>}
          {primaryFile && <p className="text-xs text-[var(--text-muted)] mt-1">Selected: {primaryFile.name}</p>}
        </div>

        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">PAN Number</label>
          <input
        ...
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">Upload PAN document</label>
          <input
            type="file"
            accept="image/*,.pdf"
          ...
          {panFile && <p className="text-xs text-[var(--text-muted)] mt-1">Selected: {panFile.name}</p>}
        </div>

        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">Driving License Number</label>
          <input
        ...
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1">Upload License document</label>
          <input
            type="file"
            accept="image/*,.pdf"
          ...
          {licenseFile && <p className="text-xs text-[var(--text-muted)] mt-1">Selected: {licenseFile.name}</p>}
        </div>
