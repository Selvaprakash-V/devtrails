      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="glass" style={{ borderRadius: '12px', padding: '1.25rem' }}>
          ...
          {SIGNALS.map(({ signal, desc, weight }) => (
            <div key={signal} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.625rem 0', borderBottom: '1px solid var(--border)' }}>
              <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: weight === 'Critical' ? '#ef4444' : '#f59e0b', flexShrink: 0 }} />
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: '0.875rem', color: 'var(--text-1)', fontWeight: 500 }}>{signal}</p>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-4)' }}>{desc}</p>
              </div>
              <span style={{ fontSize: '0.7rem', padding: '0.15rem 0.5rem', background: weight === 'Critical' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)', color: weight === 'Critical' ? '#ef4444' : '#f59e0b', borderRadius: '4px', fontWeight: 600 }}>
                {weight}
              </span>
            </div>
          ))}
